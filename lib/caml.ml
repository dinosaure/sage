...
type fd =
  ...
  | RDWR of in_channel * out_channel
...
let caml_syscall =
  ...
  let op
    ...
    | Sage.WR_ONLY -> fun path -> return (WR_ONLY (open_out path))
    | Sage.RDWR -> fun path -> return (RDWR (open_in path, open_out path)) in

  let rd
    : fd -> bytes -> off:int -> len:int -> (int, Caml_scheduler.t) io
    = function
    | RDWR (ic, _) -> fun buf ~off ~len -> return (input ic buf off len)
  ...
  let wr = function
    ...
    | RDWR (_, oc) -> fun buf ~off ~len -> output oc buf off len ; return len
    | RD_ONLY _ -> assert false in

  let close = function
    | RD_ONLY ic -> return (close_in ic)
    | WR_ONLY oc -> return (close_out oc)
    | RDWR (ic, oc) -> close_in ic ; close_out oc ; return () in
